...
struct MetadataColumnView: View {
    ...
    var body: some View {
        if let tabIndex, session.tabs.indices.contains(tabIndex) {
            ...
            // Scrolls so a record with many pages doesn't force the whole window taller than
            // the screen (which would hide the People list and status bar until a resize).
            ScrollView {
